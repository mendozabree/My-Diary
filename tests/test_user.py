...
class UserTestCase(unittest.TestCase):
    ...
    def test_API_can_not_sign_up_user_with_missing_key(self):
        ...
        response = testing_user.post('/api/v1/users',
                                     data=json.dumps(self.my_user[0]),
    ...
    def test_API_can_not_sign_up_user_with_missing_value(self):
        ...
        response = testing_user.post('/api/v1/users',
                                     data=json.dumps(self.my_users[1]),
                                     content_type='application/json')
        self.assertEqual(response.status_code, 400)
    ...
    def test_user_can_signup(self):
        ...
        response = testing_user.post('/api/v1/users',
                                     data=json.dumps(self.my_users[0]),
                                     content_type='application/json')
        self.assertEqual(response.status_code, 201)
        self.assertIn('Welcome ' + self.my_users[0]['username'],
                      str(response.data))

        res = testing_user.post('/api/v1/users',
                                data=json.dumps(self.my_users[1]),
                                content_type='application/json')
    ...
    def test_user_can_not_login_with_missing_key(self):
        ...
        response = testing_user.post('/api/v1/auth/login',
                                     data=json.dumps(self.my_user[0]),
                                     content_type='application/json')
        self.assertEqual(response.status_code, 400)
        self.assertIn('Missing username', str(response.data))
    ...
    def test_API_can_not_login_user_with_missing_value(self):
        ...
        response = testing_user.post('/api/v1/auth/login',
                                     data=json.dumps(self.my_user[1]),
                                     content_type='application/json')
        self.assertEqual(response.status_code, 400)
    ...
    def test_API_can_login_user(self):
        ...
        res = testing_user.post('/api/v1/users',
                                ...
                                content_type='application/json')
        self.assertEqual(res.status_code, 201)
        self.assertIn('Welcome ' + self.my_users[1]['username'],
                      str(res.data))

        # Login first user
        final_response = testing_user.post('/api/v1/auth/login',
                                           data=json.dumps(self.my_users[0]),
                                           content_type='application/json')
        self.assertEqual(final_response.status_code, 200)
        self.assertIn('Welcome back, ' + self.my_users[0]['username'],
                      str(final_response.data))

        # Login second user
        final_res = testing_user.post('/api/v1/auth/login',
                                      data=json.dumps(self.my_users[1]),
                                      content_type='application/json')
        self.assertEqual(final_res.status_code, 200)
        self.assertIn('Welcome back, ' + self.my_users[1]['username'],
                      str(final_res.data))
