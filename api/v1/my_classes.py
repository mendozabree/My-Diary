...
class Entry:
    ...
    def create_new_entry(self, entry_data, entries):
        ...
        If not provided an error is returned specifying the missing key or
        value or both.
        :param entry_data:
        :param entries:
        :return:
        ...
        fields_check_result = fields_check(
                                        ...
                                        pending_data=entry_data
                                          )

        if not any(fields_check_result.values()):
            entry_data['entry_id'] = self.number_of_entries \
                                   = self.number_of_entries + 1
            entries.append(entry_data)
            success_message['message'] = 'Your memory entitled ' + \
                entry_data['title'] + ' has been saved'
    ...
    def get_specific_entry(entry_id, entries):
        ...
        my_entry = [entry for entry in entries if entry['entry_id'] ==
                    entry_id]
    ...
    def modify_entry(entry_id, new_data, entries):
        ...
        current_entry = [entry for entry in entries if entry['entry_id'] ==
                         entry_id]
...
class User:
    ...
    def login(login_data, my_users):
        ...
        fields_check_result = fields_check(
                                        expected_key_list=expected_key_list,
                                        pending_data=login_data
                                          )

        if any(fields_check_result.values()) is False:
            my_user = [user for user in my_users if user['username'] ==
                       login_data['username']]
